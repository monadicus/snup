...
- [ ] Using the script, [`fund_delegator.sh`](#the-fund_delegatorsh-script), fund each unique funding address with `10_001_000_000_000` credits
- [ ] Using the script, [`verify_balance.sh`](#the-verify_balancesh-script), verify each funding address received the credits on-chain
- [ ] After all balances have been verified, delegate from each unique funding address `10_001_000_000_000` credits to each validator bonding address using the script [`delegate_to_validator.sh`](#the-delegate_to_validatorsh-script)
- [ ] Using the script, [`verify_delegation.sh`](#the-verify_delegationsh-script) verify each validator bonding address received the delegated credits on-chain
